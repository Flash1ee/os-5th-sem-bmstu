...
void start_read()
{
    ...
    // при дeкременте ждущих читателей, может начать работать писатель (потому что не произошел инкремент счетчика активных читателей)
    ...
    ReleaseMutex(mutex); // освобождение mutex
}
...
int main(void)
{
    ...
    // Wait 
    // INFINITE - пока не завершатся процессы
