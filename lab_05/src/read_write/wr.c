...
{
    return semop(s_id, START_WRITE, 6) != -1;
}
...
int reader(int *const shcntr, const int s_id, const int r_id)
{
    if (!shcntr)
    {
        return -1;
...
int writer(int *const shcntr, const int s_id, const int w_id)
{
    if (!shcntr)
    {
        return -1;
